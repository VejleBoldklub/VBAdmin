...
Fra adminfladen kan en booking desuden **annulleres**, uanset lokale, og uanset om den afventer eller allerede er bekræftet. Den får status `aflyst`, ikke `afvist`: `afvist` er svaret på en cafeteria-forespørgsel, som aldrig blev til noget, mens `aflyst` er en booking, klubben tager tilbage. Forskellen kan læses i listen bagefter, og den forsvinder, hvis begge dele hedder det samme. En allerede afvist eller aflyst booking kan ikke annulleres — der er ikke noget at tage tilbage. Bookeren får en mail om annulleringen.

Godkendelse og afvisning skrives med service_role. Afvisning kan ledsages af en begrundelse, som gemmes på bookingen og kommer med i afslagsmailen, men den er **valgfri**: er feltet tomt, gemmes `null`, og bookeren får den almindelige afslagstekst. Det gælder begge indgange — adminfladen og afvis-linket i notifikationsmailen — fordi det er den samme beslutning truffet af den samme person. Begge dele sætter `besluttet_af = 'admin'` og `besluttet_tid`. Opdateringen er betinget af, at bookingen stadig afventer, så to administratorer ikke kan overskrive hinandens beslutning.
