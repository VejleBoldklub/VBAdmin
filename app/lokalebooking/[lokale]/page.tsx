...
type LokalebookingPageProps = {
  ...
  searchParams: Promise<{ uge?: string }>;
};
...
export default async function LokalebookingPage({
  ...
  const oensketUge = (await searchParams).uge;
  ...
  const ugeSti = (noegle: string) => `/lokalebooking/${lokale.slug}?uge=${noegle}`;

  return (
    <main className="min-h-screen bg-white px-3 py-4 text-slate-950 sm:px-5 sm:py-6">
      <div className="mx-auto w-full max-w-5xl">
        {/* Ingen logo eller klubheader. Siden ligger i en iframe, hvor CMS'et
            leverer den ramme — samme valg som de offentlige baneplansider. */}
        <h1 className="text-xl font-bold tracking-tight text-slate-950 sm:text-2xl">
          Book {lokale.navn}
        </h1>
        <p className="mt-1.5 text-sm leading-6 text-slate-600">{lokale.beskrivelse}</p>

        <nav aria-label="Lokaler" className="mt-4 flex overflow-hidden rounded-lg border border-slate-200">
          {lokaler.map((l) => {
